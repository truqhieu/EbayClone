...
const AuthProductDetail = () => {
  ...
  const handleAddToCart = async () => {
    ...
    if (
      user?.role === "seller" &&
      product?.product?.sellerId?._id === user.id
    ) {
  ...
  const handleChatWithSeller = () => {
    ...
    dispatch(startConversation(product.product.sellerId._id))
      ...
      .then((result) => {
        ...
        const productInfo = `I'm interested in: ${product.product.title} ${
      ...
      <Grid container spacing={6}>
        ...
        <Grid item xs={12} md={7}>
          <Box>
            ...
            <PriceTag>${productData.price?.toFixed(2)}</PriceTag>
            ...
            <Typography
              variant="body1"
              sx={{ mt: 3, color: "#555", lineHeight: 1.7 }}
            >
            ...
            >
              <Inventory
                sx={{
                  color:
                    inventory?.quantity > 0 ? "success.main" : "error.main",
                  mr: 1.5,
                }}
              />
              <Box>
            ...
            >
              ...
              <Typography variant="body2">
                ...
                <Typography
                  ...
                  color="primary"
                  onClick={() =>
                    navigate(`/seller/profile/${productData.sellerId._id}`)
                  }
                  style={{ cursor: "pointer" }}
                >
                  {store?.storeName ||
                    productData.sellerId?.username ||
                    "Unknown Seller"}
                </Typography>
              </Typography>
            </Box>
            ...
            <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
              <ActionButton
                ...
                disabled={
                  ...
                  inventory?.quantity <= 0 ||
                  (user?.role === "seller" &&
                    productData.sellerId?._id === user.id)
                }
              >
      ...
      <Box sx={{ mt: 8 }}>
        ...
        >
          ...
          <TabPanel value={tabValue} index={0}>
            <Typography
              variant="body1"
              sx={{ whiteSpace: "pre-wrap", lineHeight: 1.8, color: "#444" }}
            >
                              ...
                              <Box sx={{ ml: 1.5, width: "100%" }}>
                                ...
                                >
                                  <Typography
                                    ...
                                    fontWeight={600}
                                  >
                                    {reply.reviewerId?.fullname ||
                                      reply.reviewerId?.username}
                                  </Typography>
                                  <Typography
                                    variant="caption"
                                    color="text.secondary"
                      ...
                      <>
                        ...
                        {store.bannerImageURL && (
                          <Box
                            ...
                            src={
                              store.bannerImageURL.startsWith("http")
                                ? store.bannerImageURL
                        ...
                        <Typography
                          variant="body1"
                          sx={{ mt: 2, mb: 3, lineHeight: 1.7, color: "#555" }}
                        >
                          {store.description ||
                    ...
                    {productData.sellerId ? (
                      <>
                        <Box sx={{ display: "flex", alignItems: "center" }}>
                          ...
                          <Box sx={{ ml: 2 }}>
                            <Typography variant="h6" fontWeight={600}>
                              {productData.sellerId.fullname ||
                                productData.sellerId.username}
                            </Typography>
                            <Typography variant="body2" color="text.secondary">
                              Member since:{" "}
                              {formatDate(productData.sellerId.createdAt)}
                            </Typography>
                          </Box>
                        </Box>
